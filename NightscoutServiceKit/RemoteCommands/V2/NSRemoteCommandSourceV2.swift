...
class NSRemoteCommandSourceV2: NSRemoteCommandSource {
    
    private let otpManager: OTPManager
    private let nightscoutClient: NightscoutUploader
    private var minimumNSCommandFormatVersion = 2.0
    
    private enum NSRemoteCommandSourceV2Error: LocalizedError {
    ...
    init(otpManager: OTPManager, nightscoutClient: NightscoutUploader) {
    ...
    func commandFromPushNotification(_ notification: [String: AnyObject]) async throws -> RemoteCommand {
        ...
        guard let pendingCommand = pendingCommands.first(where: {$0.id == notificationCommand.id}) else {
            throw NSRemoteCommandSourceV2Error.missingCommand
        }
